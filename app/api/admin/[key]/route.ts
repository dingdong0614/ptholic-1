/**
 * 관리자 전용 혼잡도 변경 페이지 (요청마다 서버에서 렌더링)
 * URL: /api/admin/<ADMIN_KEY 값>
 *
 * 정적 페이지로 만들지 않는 이유: 이 저장소는 Public GitHub 저장소라
 * 파일명에 비밀 문자열을 넣어 커밋하면 누구나 Git 기록에서 볼 수 있다.
 * 대신 이 라우트는 요청이 들어올 때마다 ADMIN_KEY 환경변수 값과 URL의 key를
...
function renderPage(adminKey: string) {
  return `<!doctype html>
<html lang="ko">
<head>
<meta charset="UTF-8" />
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ key: string }> }) {
  ...
  return new Response(renderPage(adminKey), {
    status: 200,
    headers: {
